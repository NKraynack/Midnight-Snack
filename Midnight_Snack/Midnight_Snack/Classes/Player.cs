...
namespace Midnight_Snack
{
    public class Player : MobileUnit
    {
        ...
        public override void SetMovedThisTurn(bool b)
        {
            if (b == true)
            {
                this.UndrawTile();
                prev_col = this.GetCol();
                prev_row = this.GetRow();
            }
            movedThisTurn = b;
        }
        ...
        {
            int player_x = this.GetCol();
            int player_y = this.GetRow();
            for (int i = player_x - this.GetMoveRange(); i <= player_x + this.GetMoveRange(); i++)
            {
                for (int j = player_y - this.GetMoveRange(); j <= player_y + this.GetMoveRange(); j++)
                {
                    Console.WriteLine("hi " + i + " " + j + " " + this.GetMoveRange());
                    try
                    {
                        if ((i != player_x || j != player_y && this.NoObstacles(i, j)) && NoObstacles(i, j))
                        {
                            if (i == map.GetLairCol() && j == map.GetLairRow())
                            {
                                MapTile ob = map.GetTile(j, i);
                                ob.SetModifier("move_lair");
                                map.SetTile(j, i, ob);
                            }
                            else
                            {
                    ...
                    {
                        Console.WriteLine("out of range, supress exception...");
                    }
                }
            }
        }
        ...
        {
            int player_x = this.prev_col;
            int player_y = this.prev_row;
            for (int i = player_x - this.GetMoveRange(); i <= player_x + this.GetMoveRange(); i++)
            {
                for (int j = player_y - this.GetMoveRange(); j <= player_y + this.GetMoveRange(); j++)
                {
                    Console.WriteLine("hello " + i + " " + j + " " + this.GetMoveRange());
                    try
                    {
                        if (i == map.GetLairCol() && j == map.GetLairRow())
                        {
